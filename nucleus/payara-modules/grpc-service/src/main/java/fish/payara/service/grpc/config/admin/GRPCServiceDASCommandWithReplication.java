 ...
 * @author steve
 */
@Service(name = "grpcservice-das-command") // the name of the service is the asadmin command name
...
@RestEndpoints({  // creates a REST endpoint needed for integration with the admin interface
    @RestEndpoint(configBean = GRPCServiceConfiguration.class,
            ...
            path = "grpcservice-das-command",
            description = "Runs a command on the DAS first")
})
public class GRPCServiceDASCommandWithReplication implements AdminCommand {
    ...
    public void execute(AdminCommandContext context) {
        ...
        ClusterOperationUtil.replicateCommand("grpcservice-instance-command", FailurePolicy.Ignore, FailurePolicy.Ignore, FailurePolicy.Error, domainServers, context, params, habitat);
