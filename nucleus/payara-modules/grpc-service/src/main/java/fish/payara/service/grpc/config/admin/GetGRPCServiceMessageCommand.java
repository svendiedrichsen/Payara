 ...
 * @author steve
 */
@Service(name = "get-grpcservice-message") // the name of the service is the asadmin command name
...
@RestEndpoints({  // creates a REST endpoint needed for integration with the admin interface
    @RestEndpoint(configBean = GRPCServiceConfiguration.class,
            ...
            path = "get-grpcservice-message",
            description = "Gets the Service Configuration")
})
public class GetGRPCServiceMessageCommand implements AdminCommand { // must implement AdminCommand to be an asadmin command
    ...
    public void execute(AdminCommandContext context) {
        ...
        GRPCServiceConfiguration serviceConfig = configNode.getExtensionByType(GRPCServiceConfiguration.class);

        // add return message
        context.getActionReport().setMessage("gRPC Service port is " + serviceConfig.getPort());
    }
