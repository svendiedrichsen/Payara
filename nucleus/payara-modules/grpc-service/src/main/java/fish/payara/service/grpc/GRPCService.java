 ...
 * file and include the License file at packager/legal/LICENSE.txt.
  */
package fish.payara.service.grpc;
 ...
 */
@Service(name = "grpc-service") // this specifies that the class is an HK2 service
@RunLevel(StartupRunLevel.VAL)  // this specifies the service is created at server boot time
public class GRPCService implements EventListener, MessageReceiver, ConfigListener {

    private static final Logger LOGGER= Logger.getLogger(GRPCService.class.getCanonicalName());
    ...
    // @Inject
    // PayaraExecuterService executerService;

    // This injects the configuration from the domain.xml magically
    // and for the correct server configuration
    @Inject
    ...
    public void bootService() {
        LOGGER.info("gRPC Service starts booting...");
        ...
        // based on Hazelcast
        eventBus.addMessageReceiver("gRPCService", this);
        eventBus.publish("gRPCService", new ClusterMessage("Hello World"));

        // notify that I am interested in changes to the configuration class
        transactions.addListenerForType(GRPCServiceConfiguration.class, this);

        if (config.isEnabled()) {
            grpcServer = ServerBuilder
                    .forPort(config.getPort())
                    .build();
            LOGGER.info("gRPC Service has booted on port " + config.getPort());
        } else {
            LOGGER.info("gRPC Service disabled.");
        }
    ...
    public void reconfigure() {
        ...
        LOGGER.info("Config now says " + config.getPort());
    }
